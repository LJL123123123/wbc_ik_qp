...
target_pos = {
            ...
            "LH": torch.tensor([-0.25, 0.15, -height], device=device, dtype=dtype),
            "LF": torch.tensor([0.14, 0.15, -height], device=device, dtype=dtype),
            "RF": torch.tensor([0.14, -0.15, -height], device=device, dtype=dtype),
            "RH": torch.tensor([-0.25, -0.15, -height], device=device, dtype=dtype),
        }
target_ori = {
...
def loop():
    ...
    target_ori["com"] = torch.tensor(quaternion2rot(quaternion), device=device, dtype=dtype)
    h = 0.1 / -0.25
    l = 0.1
    if t%2.0 < 1.0:
        target_pos["LF"][0] = 0.14 + -l * (t % 2.0) + 0.2 + target_pos["com"][0]
        target_pos["LF"][1] = 0.15
        target_pos["LF"][2] = -height + height

        target_pos["RH"][0] = -0.25 + -l * (t % 2.0) + 0.1 + 0.1 + target_pos["com"][0]
        target_pos["RH"][1] = -0.15 
        target_pos["RH"][2] = -height + height

        target_pos["LH"][0] = -0.25 + l * (t % 2.0) + 0.1 + target_pos["com"][0]
        target_pos["LH"][1] = 0.15 
        target_pos["LH"][2] = -height + h*(t % 2.0)*(t % 2.0-1.0) + height

        target_pos["RF"][0] = 0.14 + l * (t % 2.0) - l +0.2 + target_pos["com"][0]
        target_pos["RF"][1] = -0.15 
        target_pos["RF"][2] = -height + h*(t % 2.0)*(t % 2.0-1.0) + height

    else :
        target_pos["LH"][0] = -0.25 + -l * (t % 2.0 - 1.0) +l +0.1 + target_pos["com"][0]
        target_pos["LH"][1] = 0.15  
        target_pos["LH"][2] = -height + height

        target_pos["RF"][0] = 0.14 + -l * (t % 2.0 - 1.0) +0.2 + target_pos["com"][0]
        target_pos["RF"][1] = -0.15 
        target_pos["RF"][2] = -height + height

        target_pos["LF"][0] = 0.14 + l * ((t % 2.0) - 1.0) - l +0.2 + target_pos["com"][0]
        target_pos["LF"][1] = 0.15 
        target_pos["LF"][2] = -height + h*((t % 2.0) - 1.0)*((t % 2.0)-1.0 - 1.0) + height

        target_pos["RH"][0] = -0.25 + l * ((t % 2.0) - 1.0) - l + 0.1 +0.1 + target_pos["com"][0]
        target_pos["RH"][1] = -0.15 
        target_pos["RH"][2] = -height + h*((t % 2.0) - 1.0)*((t % 2.0) - 1.0 - 1.0) + height
